...
| 2026-09-26 | Opus → astra | 蓄压罐"动力不够时每秒最多补 3"在 `battle.js` 里是写死的 `3`，车间说明也只能写死。建议提成常量（如 `SA.K.STORE_RELEASE`）或模块字段，Opus 再改成读它 | 待做 |
...
| W 战役方向配套 | `docs/campaign-direction.md` §10：跳弹反馈、修理费呈现、支线入口、重打入口、双足唯一件画面、早期瞄准手感 | W1 跳弹反馈、W2 修理费呈现完成；W3～W5 等 astra；W6 待做 |
