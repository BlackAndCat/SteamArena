...
| 2026-09-26 | Opus → astra | **双足占格和 `docs/true-biped.md` §8 对不上**：§8 定的是双足占「最下面两**层**」（大格：胯一层 + 腿区一层 = 4 个子格行，样机 `bipedArt` 是 2×4 子格、48×96），现在的数据是 `w: 1, h: 2`（子格，胯和腿区各只有 1 个子格行、24 宽），腰挂位也按子格算。请按 §8 改成层（或和用户确认后改 §8），Opus 再接真双足画面 | 待做（真双足画面暂缓） |
...
| 项 | 内容 | 状态 |
|---|---|---|
| V1 底盘画面接入 | 四足整件（一整块甲壳 + 四条腿、按距离的大步态、四脚悬挂）；真双足：胯和一对腿、躯干切角和姿态错位、跪地 / 陀螺停转的表现、编辑器平衡指示 | 四足完成；真双足等占格问题（见交接板） |
