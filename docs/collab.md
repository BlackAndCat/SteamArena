...
| E 真双足 | `docs/true-biped.md` §8 的数据、规则、平衡系统、踢击、迁移（渲染归 Opus） | 完成（`ef5cd75`；渲染待 Opus） |
| G 分享码车库 | 云车库改成纯分享码导入 / 导出，去掉"上传到云端"的模拟。友谊赛并入重打（K7）；导入的车在哪里打待定 | 待做 |
| K 战役方向 | `docs/campaign-direction.md` §9：K1 跳弹、K2 反震、K3 修理费比例、K4 战役重排、K5 唯一件、K6 支线、K7 重打、K8 进化生成器联动、K9 早期近战验证。K1～K3 属于战斗规则，要在冻结点之前完成 | 进行中（K1～K4 第一版完成，待用户审阅；K5～K9 待做） |
...
建议顺序：A + A′（完成）→ F + J-P0 → J-P1 → K1～K3（跳弹、反震、修理费）→ J-P2（系统健康测试，含 K9）→ E（完成）→ K4（第一版完成，待审）→ K5～K7、G → J-P3～P8（含 K8）→ 冻结点后正式生成 → C / D → H。
